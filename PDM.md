...
2. 创建一个虚拟环境 `-m venv` 表示使用该解释器的 `venv` 模块；`test` 是虚拟环境名字执行完毕后，会创建一个 test 文件夹。若需要移除该虚拟环境，只需要删除这个文件夹。
...
3. pipx 和 pip 最大的区别就是前者是全局安装的，而 pip 的包只在当前环境可用
...
### 特点总结
1. PDM 用于管理软件包，它不会创建 python 解释器，它要求用户在初始化项目的时候选择一个本机已有的 python 解释器，然后创建该解释器的软链接到 `.venv` 中
2. 如果先激活了一个 conda 环境，且该环境和 `pyproject.yml` 中的解释器版本一样，那么 pdm 就不会创建当前项目下的.venv 而是把包放在当前 conda 环境下
3. 若选择的解释器是 conda 默认的 base 环境，则 pdm 还是会创建环境，因为 miniforge 的系统解释器，不算虚拟环境
...
`cd` 到项目路径，执行以下代码，完毕后只是会出现 `pyproject.toml` 和 `.venv`，并不会自动创建用 pdm 管理的包

```
pdm init
# 使用指定的 python 解释器创建 pdm，因为默认只有 conda 的 base 环境可被选择
pdm init --python /opt/miniforge3/envs/test/bin/python
```
也可以
```
conda activate test
...
也可以一次性安装许多个依赖，由空格隔开
```
pdm add "torch>=2.6.0" transformers scikit-learn pandas numpy tqdm openpyxl --index https://pypi.org/simple
```
或者在 pdm 中通过 pip 安装
```
pdm run pip install torch --index-url https://download.pytorch.org/whl/cpu
```
