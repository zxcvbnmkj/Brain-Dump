...
```
## 使用 ES 用作网页的模糊搜索
**ES 主要用来增加索引准确度，而不是提高效率的，使用它似乎不会提效**
- 创建新索引
- 把数据库中一些必要的字段（title、content等）同步到数据库中，createTime 等不用于搜索的字段不同步
- 对于用户 query 先去 ES 中搜索，若搜索不到，再去数据库搜索
- 对于 ES 中搜到的内容，返回它的 id，再用 id 去数据库中取完整的一条数据，然后返回
